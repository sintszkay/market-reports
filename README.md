...
- Report ETF/RSI source tables are limited to the current three workbooks/tabs: `Sector Dashboard`, `Thematic Sectors`, and `Macro`. The legacy hidden `Market` tab must not be restored as a report table or cited as the current primary source. The four major ETFs may still appear in the separate `指數與風格收盤` analytical summary.
