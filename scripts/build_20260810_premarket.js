...
const adjustedSnapshot = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', '2026-08-10-longbridge-adjusted.json'), 'utf8'));
const quoteSnapshot = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', '2026-08-10-longbridge-quotes.json'), 'utf8'));
const sheetSnapshot = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', '2026-08-10-google-sheet.json'), 'utf8'));
...
Object.assign(pre, {
  B:{ticker:'B',price:41.45,previousClose:43.68,changePct:-5.1053,volume:301208,turnover:12573728.51,timestamp:'2026-08-10T12:07:40Z',premarketAvailable:true},
  MNDY:{ticker:'MNDY',price:84.50,previousClose:93.13,changePct:-9.2666,volume:183858,turnover:15536001,timestamp:'2026-08-10T12:07:40Z',premarketAvailable:true}
});
...
const moverTickers = ['COHR','SMCI','RKLB','AMAT','LRCX','KLAC','META','MRVL','INTC','MNDY','B','TTD','NET','PLTR','AAPL','MU'];
...
const checklist = [
  ['S&amp;P 500 overextension／大盤過度延伸','High',`SPY +${n(adjusted.SPY.distance50Atr)} ATR；DIA +${n(adjusted.DIA.distance50Atr)} ATR`,'DIA 接近 +3 ATR，SPY 亦處高延伸；趨勢未壞但追價回報下降。','high'],
  ['Increasing downward momentum／下行動能增加','Low',`QQQ ${pct(pre.QQQ.changePct)}；SMH ${pct(pre.SMH.changePct)}`,'指數近乎平盤、半導體偏強，未見下行動能擴散。','low'],
  ['Top range breakdown／高位區間破位','Low','四大 ETF 均高於 20／50／200MA','上週五收盤沒有高位區間破位。','low'],
  ...
  ['Breakout win rate down／突破勝率下降','Low','Stockbee 5D 2.85；10D 1.64','兩個延續率高於 1，4% 上漲股 510 明顯多於下跌股 152。','low'],
  ['Theme momentum weakening／主題動能轉弱','Intermediate',`XSW RSI ${n(sheetTech.XSW.rsi14)}；MNDY ${pct(pre.MNDY.changePct)}`,'軟體 ETF 強，但 MNDY、TTD、NET、PLTR 盤前偏弱，個股分化升高。','mid']
];
const checklistHtml = `<div class="risk-check-grid">${checklist.map(row => `<div class="risk-check-row ${row[4]}"><div class="risk-check-name">${row[0]}</div><div class="risk-check-level">${badge(row[1], row[4] === 'high' ? 'red' : row[4] === 'mid' ? 'amber' : 'green')}</div><div class="risk-check-reading"><strong>${row[2]}</strong><small>${row[3]}</small></div></div>`).join('')}</div><div class="callout warn"><strong>Checklist：1/8 High。</strong>廣度與波動仍支持風險資產，但指數延伸、油價跳升及 CPI 前事件風險提高追價門檻。</div>`;

const macroEvents = table(['宏觀／財報事件','Actual','Forecast','Previous','訊號'], [
  [td('<span class="macro-event"><strong>今日美國高重要數據</strong><small>8/10｜盤前</small></span>'),numTd('無'),numTd('無'),numTd('—'),td(badge('價格主導','blue'))],
  [td('<span class="macro-event"><strong>NFIB 小型企業信心</strong><small>8/11｜06:00 ET</small></span>'),numTd('待公布'),numTd('未提供'),numTd('97.4'),td(badge('次要','blue'))],
  ...
  [td('<span class="macro-event"><strong>初領失業金</strong><small>8/13｜08:30 ET</small></span>'),numTd('待公布'),numTd('201K'),numTd('199K'),td(badge('勞動驗證','blue'))],
  [td('<span class="macro-event"><strong>B 財報</strong><small>8/10｜盤前已公布</small></span>'),numTd('EPS 0.73<br>營收 5.292B'),numTd('EPS 0.81<br>營收 5.083B'),numTd('—'),td(badge('Miss／Beat','amber'))],
  [td('<span class="macro-event"><strong>MNDY 財報</strong><small>8/10｜盤前已公布</small></span>'),numTd('營收 364.6M<br>現金流下降'),numTd('營收 356M'),numTd('—'),td(badge('Beat／Mixed','amber'))],
  [td('<span class="macro-event"><strong>HIMS／RKLB 財報</strong><small>8/10｜盤後</small></span>'),numTd('待公布'),numTd('HIMS EPS -0.05<br>RKLB 營收約 231M'),numTd('—'),td(badge('事件風險','blue'))]
], 'report-data-table macro-results-table', [1, 2, 3]);

const priorReviewRows = [
  [td('<strong>利率下行利多 AI 硬件</strong><small>以 QQQ／SMH 與設備鏈共同驗證。</small>'),td('8/7 QQQ +1.17%、SMH +1.96%；設備與光通訊同步走強。'),td(badge('命中','green')),td('今日仍需 SMH 收回 50MA 才由短線反彈升級。')],
  [td('<strong>TLT 守位、10Y 不回 4.67%</strong><small>長久期反彈的必要條件。</small>'),td('10Y 收約 4.64%，TLT +0.29%，條件成立。'),td(badge('命中','green')),td('今日油價上升使 TLT 盤前轉弱，重新驗證。')],
  [td('<strong>油銅弱代表增長代價</strong><small>不把 QQQ 上漲等同全面 risk-on。</small>'),td('8/7 CPER -2.11%、USO -0.75%，但 IWM +1.11%；訊號出現但未拖累指數。'),td(badge('已觸發','amber')),td('將商品與股票分開記錄，等待是否形成同向壓力。')],
  [td('<strong>財報巨幅缺口不追第一段</strong><small>以 VWAP 與首小時驗證。</small>'),td('TTD 最終大跌約 22%，負缺口未快速修復；規則有效。'),td(badge('命中','green')),td('今日套用至 MNDY、B 與盤後 HIMS／RKLB。')],
  [td('<strong>增長警報限制小型股</strong><small>弱非農可能壓制 IWM。</small>'),td('IWM +1.11%，並未受油銅下跌拖累；風險提示方向偏保守。'),td(badge('偏保守','blue')),td('不再用單一宏觀警報提前否定價格與廣度。')]
];
const priorPremarketReview = `<section class="prior-premarket-review"><h2>上次盤前判斷複盤（8/7）</h2>${table(['8/7 盤前主判斷','8/7 收盤事實','對賬','今日修正'], priorReviewRows, 'report-data-table premarket-review-table')}<div class="callout warn"><strong>對賬：3 命中、1 已觸發、1 偏保守、0 失誤。</strong>利率與 AI 硬件主線成立，但油銅弱並未演變成指數去風險。</div><p class="section-summary"><strong>本段結論：</strong>今日先保留硬件相對強勢；油價反彈與 TLT 轉弱要求重新驗證長久期條件。</p></section>`;

const major = ['IWM','DIA','SPY','QQQ'].map(ticker => {
  const row = requireRow(ticker);
  const quote = requirePre(ticker);
  const notes = {
    IWM:`高於 20／50／200MA；盤前略低於前收 ${n(row.close)}，油價升溫是今日逆風。`,
    DIA:`距 50MA ${n(adjusted.DIA.distance50Atr)} ATR，四大 ETF 中延伸最高；只持有不追。`,
    SPY:`高於三條均線，盤前貼近前收 ${n(row.close)}；以 VWAP 判斷是否消化高延伸。`,
    QQQ:`高於 20／50／200MA；盤前略強，守 50MA ${n(row.ma50)} 才保留科技主線。`
  };
...
const breadthRows = [
  ['SPX >20MA','65.20%','63.02% → 65.20%','53.28% → 65.20%','短中線同步擴散。'],
  ['SPX >50MA','65.60%','64.21% → 65.60%','62.02% → 65.60%','中期維持六成以上。'],
  ['NDX >20MA','70.58%','61.76% → 70.58%','53.39% → 70.58%','科技短線廣度顯著改善。'],
  ['NDX >50MA','52.94%','51.96% → 52.94%','47.57% → 52.94%','回到五成以上但緩衝仍薄。'],
  ['IWM >20MA','63.83%','60.45% → 63.83%','43.93% → 63.83%','小型股五日改善最大。'],
  ['IWM >50MA','62.96%','60.14% → 62.96%','52.88% → 62.96%','中期參與度偏多。'],
  ['T2108','53.58%','52.52% → 53.58%','47.36% → 53.58%','中性偏多並重新升溫。'],
  ['Stockbee 5D ratio','2.85','2.28 → 2.85','0.98 → 2.85','延續率明顯高於 1。'],
  ['Stockbee 10D ratio','1.64','1.37 → 1.64','0.91 → 1.64','中短線續改善。'],
  ['4%+ 上漲／下跌','510／152','300／280 → 510／152','179／216 → 510／152','上漲家數明顯領先。'],
  ['季度 +25%／-25%','1540／1051','1410／1087 → 1540／1051','1171／1235 → 1540／1051','中期強股優勢擴大。'],
  ['單月 +50%／-50%','27／34','23／37 → 27／34','13／33 → 27／34','正尾改善，負尾仍偏高。']
];
...
const dxyRow = {ticker:'DXY',close:99.725,dailyPct:0.1255,fiveDayPct:-0.2351,oneMonthPct:-1.2330,rsi14:28.12,above20:false,above50:false,above200:false};
...
const fxTable = `<div class="macro-policy-overview"><div><span>DXY</span><strong class="dn">99.73</strong><small>5日 -0.24%／RSI 28.12</small></div><div><span>原油代理</span><strong class="up">USO ${pct(usoPre.changePct)}</strong><small>地緣溢價回升</small></div><div><span>黃金</span><strong class="dn">GLD ${pct(gldPre.changePct)}</strong><small>油價與利率壓力</small></div></div>${table(['資產','8/7收盤','1日','5日','1月','8/10盤前','RSI','趨勢／RSI 含義'], fxRows, 'report-data-table fx-trend-table fx-trend-table-8', [1, 2, 3, 4, 5, 6])}`;

const bondLabels = {SHY:'1–3年短債',IEF:'7–10年中債',TLT:'20年以上長債'};
const bondRows = ['SHY','IEF','TLT'].map(ticker => {
  const quote = requirePre(ticker);
  const signal = ticker === 'TLT' ? '油價跳升令長端領跌，通膨期限溢價重新壓制久期。' : ticker === 'IEF' ? '中段同步回落，但跌幅小於長債。' : '短債近乎持平，政策路徑尚未出現同等幅度重定價。';
...
const expectedTable = `<div class="callout warn"><strong>本週預期波動尚未更新。</strong>Google Sheet 目前仍是 8/3–8/7 區間，已過期，因此不把舊 +1SD／+2SD 數值當成 8/10 交易門檻；本稿改用 20MA、50MA、VWAP 與前收盤。</div>`;

const tradeActions = {
  DIA:`接近 +3 ATR；守前收 ${n(close.DIA.close)}，只持有不追價。`,
  SPY:`守前收 ${n(close.SPY.close)}；跌破 20MA ${n(close.SPY.ma20)} 才升級結構風險。`,
  IWM:`守前收 ${n(close.IWM.close)}；油價升而小盤轉弱時降低景氣曝險。`,
  QQQ:`守前收 ${n(close.QQQ.close)} 與 50MA ${n(close.QQQ.ma50)}；CPI 前不追第二段。`,
  SMH:`先看 50MA ${n(close.SMH.ma50)}，配合 COHR／AMAT／LRCX／KLAC 驗證硬件共振。`,
  USO:'盤前跳升逾 3%；未回落至 120 下方前，保留通膨溢價風險。',
  TLT:`盤前轉跌；守 ${n(close.TLT.close)} 與 VWAP，否則久期主線降級。`
};
...
fs.writeFileSync(path.join(ROOT, 'data', '2026-08-10-premarket.json'), JSON.stringify(data, null, 2), 'utf8');
fs.writeFileSync(path.join(ROOT, 'reports', '2026-08-10-premarket-update.html'), html, 'utf8');
console.log(JSON.stringify({report:'reports/2026-08-10-premarket-update.html',sectorRows:sectors.length,thematicRows:thematic.length,movers:moverTickers.length,majorEtf:major.length,checklist:checklist.length,technicalScore,breadthScore,vixScore,unresolved:unresolved.length}, null, 2));
