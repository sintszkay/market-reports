...
const OUTPUT = path.join(ROOT, 'data', QUOTES_ONLY ? '2026-08-14-longbridge-quotes.json' : ADJUST === 'forward' ? '2026-08-14-longbridge-adjusted.json' : '2026-08-14-longbridge.json');
const CLI = '/opt/homebrew/bin/longbridge';
const AS_OF = '2026-08-13';
const PREMARKET_DATE = '2026-08-14';
...
const moverCandidates = [
  ...
  'PINS','RBLX','RCEL','RIVN','RKLB','SEZL','SHOP','SMCI','SNDK','SNOW','TSLA','TTD','U','UBER','WMT','MSTR','ASTS','AMAT','CRM','NOW','ADBE','ONTO','PRAA'
];
...
function parseJsonObject(stdout) {
  const start = stdout.indexOf('{');
  const end = stdout.lastIndexOf('}');
  if (start < 0 || end < start) throw new Error('Longbridge output did not contain a JSON object');
  return JSON.parse(stdout.slice(start, end + 1));
}
...
async function fetchTicker(ticker) {
  const {stdout} = await retry(() => execFileAsync(CLI, [
    'kline','history',`${ticker}.US`,'--period','day','--start','2025-08-01','--end','2026-08-13',
...
async function fetchTopMovers() {
  const {stdout} = await retry(() => execFileAsync(CLI, [
    'top-movers','--market','US','--sort','change','--count','50','--format','json'
  ], {maxBuffer: 20 * 1024 * 1024}));
  const payload = parseJsonObject(stdout);
  return (payload.events || []).map(event => ({
    ticker: event.stock?.code || null,
    name: event.stock?.full_name || event.stock?.name || null,
    price: number(event.stock?.last_done),
    changePct: round(number(event.stock?.change) * 100),
    overnightPrice: event.stock?.overnight_price ? number(event.stock.overnight_price.last_done) : null,
    overnightChangePct: event.stock?.overnight_price ? round(number(event.stock.overnight_price.change) * 100) : null,
    alertReason: event.alert_reason || null,
    newsTitle: event.post?.title_locale?.hk || event.post?.title_locale?.original || event.post?.title || null,
    newsSummary: event.post?.desc_locale?.hk || event.post?.desc_locale?.original || event.post?.description_html || null,
    newsUrl: event.post?.source_url || event.post?.web_url || null,
...
async function main() {
  ...
  const topMovers = TECHNICAL_ONLY ? [] : await fetchTopMovers();
  const technical = QUOTES_ONLY ? {rows: [], errors: []} : await runPool(technicalTickers, 6, fetchTicker);
  ...
  const output = {
    ...
    source: `Longbridge CLI ${[
      TECHNICAL_ONLY ? null : 'quote + top-movers',
      QUOTES_ONLY ? null : `kline history --adjust ${ADJUST}`
    ].filter(Boolean).join(' + ')}`,
    counts: {
      technicalRequested: QUOTES_ONLY ? 0 : technicalTickers.length,
