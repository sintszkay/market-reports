...
const BASELINE = JSON.parse(fs.readFileSync(path.join(ROOT, "data", "2026-08-21-weekly-longbridge.json"), "utf8"));
const OUTPUT = path.join(
  ROOT,
  "data",
  ADJUST === "forward"
    ? "2026-08-28-weekly-longbridge-adjusted.json"
    : "2026-08-28-weekly-longbridge.json"
);

const universes = BASELINE.universes;
const tickers = BASELINE.rows.map((row) => row.ticker);
const coreTickers = new Set(universes.coreTickers);
...
function summarize(ticker, rawCandles) {
  ...
  if (coreTickers.has(ticker) && candles.length < 200) throw new Error(`${ticker} has only ${candles.length} candles for core technicals`);
  ...
  return JSON.parse(JSON.stringify(result), (key, value) => typeof value === "number" ? round(value) : value);
}
...
async function main() {
  const technical = await runPool(tickers, 6, fetchTicker);
  const output = {
    generatedAt: new Date().toISOString(),
    asOf: AS_OF,
    source: `Longbridge CLI kline history --adjust ${ADJUST}`,
    counts: { requested: tickers.length, success: technical.rows.length, errors: technical.errors.length },
    universes,
    rows: technical.rows,
