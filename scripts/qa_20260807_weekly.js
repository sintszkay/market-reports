...
const file = process.argv[2] || "reports/2026-08-07-weekly.html";
...
need(/廣度全面修復，科技領漲/, "主標題未呈現本週核心敘事");
need(/25<small>\/100<\/small>/, "市場總分 25/100 缺失");
need(/Low Risk/, "Low Risk 分類缺失");
need(/3 命中/, "上週對賬命中數缺失");
need(/1 已觸發/, "上週對賬已觸發數缺失");
need(/5 未觸發/, "上週對賬未觸發數缺失");
...
need(/5日惡化 0\/8/, "五日廣度量化分數缺失");
need(/技術 0\/12/, "三大指數技術分數缺失");
need(/VIX 0\/5/, "VIX 五項分數缺失");
need(/DXY[\s\S]{0,220}99\.54/, "DXY 8/7 正式收盤缺失");
need(/2Y／10Y／30Y 一週下降 9／10／8bp/, "美債週變化缺失");
need(/FOMC 目標區間/, "FOMC 目標區間缺失");
need(/9\/16 FOMC 市場定價/, "下次 FOMC 市場定價缺失");
need(/7月非農就業/, "本週就業復盤缺失");
need(/8\/12 08:30/, "下週 CPI 事件缺失");
need(/8\/13 08:30/, "下週 PPI 事件缺失");
need(/8\/14 08:30/, "下週零售事件缺失");
...
if (scoreCells.reduce((sum, row) => sum + row[0], 0) !== 25 || scoreCells.reduce((sum, row) => sum + row[1], 0) !== 100) failures.push("市場分項無法反算為 25/100");
