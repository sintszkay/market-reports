...
const reportPath = path.join(OUTPUTS, '2026-08-14-premarket-update.html');
    ...
    for (const viewport of viewports) {
      ...
      const metrics = await page.evaluate(() => {
        ...
        for (const parent of document.querySelectorAll('.summary-grid,.risk-check-grid,.macro-policy-overview')) {
          const children = [...parent.children].map(element => ({rect:element.getBoundingClientRect()})).filter(item => item.rect.width > 0 && item.rect.height > 0);
          for (let i = 0; i < children.length; i += 1) {
            for (let j = i + 1; j < children.length; j += 1) {
              ...
              if (Math.min(a.right, b.right) - Math.max(a.left, b.left) > 2 && Math.min(a.bottom, b.bottom) - Math.max(a.top, b.top) > 2) {
                overlapPairs.push(`${parent.className}:${i}-${j}`);
              }
            }
          }
        }
        ...
        return {
          ...
          overlapPairs,
          tableMismatches,
          escapedTables:tableContainers.filter(container => {
      ...
      const screenshot = path.join(OUTPUTS, `qa-2026-08-14-premarket-${viewport.name}.png`);
