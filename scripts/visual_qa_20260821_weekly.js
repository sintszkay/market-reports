...
const reportPath = path.join(outputs, "2026-08-21-weekly.html");
    ...
    for (const viewport of viewports) {
      ...
      const metrics = await page.evaluate(() => {
        ...
        const macroFedLayout = macroFedTable ? (() => {
          ...
          const overflowCells = [...macroFedTable.querySelectorAll("th,td")].filter((cell) => cell.scrollWidth > cell.clientWidth + 1);
          return {
            ...
            overflowCellDetails:overflowCells.map((cell) => ({text:cell.textContent.trim(),clientWidth:cell.clientWidth,scrollWidth:cell.scrollWidth})),
            maxRowHeight:Math.max(0, ...rows.map((row) => row.getBoundingClientRect().height)),
      ...
      const screenshot = path.join(outputs, `qa-2026-08-21-weekly-${viewport.name}.png`);
